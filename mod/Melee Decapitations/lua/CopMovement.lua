...
Hooks:PreHook( CopMovement , "update" , "MeleeDecapCopMovementPreUpdate" , function( self , unit , t , dt )
	...
	if self._dismemberments then
		if self._dismemberments.Head then
			...
			if MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ] and MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ][ 2 ] then
				local l = {
				...
				self._dismemberments.fx = World:effect_manager():spawn({
					effect = Idstring( MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ][ 2 ] ),
					position = unit:get_object( Idstring( "Neck" ) ):position(),
					rotation = self._dismemberments.variant == "LargeBladed" and l[ MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ][ 3 ] ] or self._dismemberments.variant == "SmallBladed" and s[ MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ][ 3 ] ] or l[ MeleeDecap.MenuOptions.MultipleChoice[ "SpurtEffect" ][ 4 ][ MeleeDecap:HasSetting( "SpurtEffect" ) ][ 3 ] ]
				})
			end
