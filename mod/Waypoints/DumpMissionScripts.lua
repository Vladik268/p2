...
local file = io.open(mission..".txt", "w")
file:close()
local file = io.open(mission..".txt", "a+")
						...
						if val2:sub(1,6) == '_value' then
							...
							for k3, v3 in pairs (v2) do
								...
								if val3:sub(1,12) == 'trigger_list' or val3:sub(1,11) == 'on_executed' or val3:sub(1,8) == 'elements' then
									...
									if val3:sub(1,11) == 'on_executed' and tostring(managers.mission:script(name)._elements[k1]._values.on_executed[1]) == 'nil' then
										file:write("\t\t\t\t\tnil\n")
									end
